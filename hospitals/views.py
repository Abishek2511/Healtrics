...
def edit_profile(request):
    if request.method == 'POST':
        ...
        coords = get_coordinates(request.POST['address'] + "," + request.POST['city'] + "," + request.POST['state'] + "," + request.POST['country'])
...
def request_for_blood(request):
    if request.method == 'POST':
        ...
        for blood_bank in blood_banks:
            ...
            sendMail("Healtrics Blood Request!!!","You got a blood request!!! Please visit your profile!!!",settings.EMAIL_HOST_USER,[blood_bank.user.email_address])
            sendSMS("Healtrics !!!You got a blood request!!! Please visit your profile!!!",blood_bank.user.mobile_number)
...
def view_accepted_donors(request,request_id):
    blood_request_donors = BloodRequestDonor.objects.filter(blood_request__request_id=request_id).all()
...
def view_accepted_blood_banks(request,request_id):
    blood_request_blood_banks = BloodRequestBloodBank.objects.filter(blood_request__request_id=request_id).all()
    return render(request,'hospitals/view_accepted_blood_banks.html',{'blood_request_blood_banks':blood_request_blood_banks})

@login_required
@user_passes_test(test_func=check_hospitals,login_url='/accounts/hospitals_login')
def view_accepted_blood_bank_inventory(request,blood_bank_id):
    ...
    return render(request,'hospitals/view_accepted_blood_bank_inventory.html',{'blood_bank_inventory':blood_bank_inventory})
